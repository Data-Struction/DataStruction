...
def search_kids_and_spouse(idx,clist,count):
    if tb.family[idx].spouse != -1:
        spouse = tb.family[idx].spouse
        clist.append(bs.alist[spouse - count])
        del bs.alist[spouse - count]
        count += 1
    if tb.family[idx].kids == []:
        return clist,count
    else:
        for i in range(len(tb.family[idx].kids)):
            ...
            clist.append(bs.alist[kid - count])
            del bs.alist[kid - count]
            count += 1
            search_kids_and_spouse(kid,clist,count)
